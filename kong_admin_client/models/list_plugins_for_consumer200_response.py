...
class ListPluginsForConsumer200Response(BaseModel):
    ...
    route: Optional[StrictStr] = Field(default=None, description="If set, the plugin will only activate when receiving requests via the specified route. Leave unset for the plugin to activate regardless of the route being used. With form-encoded, the notation is `route.id=<route id> or route.name=<route name>`. With JSON, use `\"route\":{\"id\":\"<route id>\"}` or `\"route\":{\"name\":\"<route name>\"}`.")
    ...
    __properties: ClassVar[List[str]] = ["id", "name", "created_at", "route", "service", "consumer", "instance_name", "config", "protocols", "enabled", "tags", "ordering"]
