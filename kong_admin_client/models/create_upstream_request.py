...
class CreateUpstreamRequest(BaseModel):
    ...
    """ # noqa: E501
    name: StrictStr = Field(description="This is a hostname, which must be equal to the `host` of a service.")
    algorithm: Optional[StrictStr] = Field(default='round-robin', description="Which load balancing algorithm to use. Accepted values are: `\"consistent-hashing\"`, `\"least-connections\"`,` \"round-robin\"`. Default: `\"round-robin\"`. ")
    ...
    slots: Optional[Annotated[int, Field(le=65536, strict=True, ge=10)]] = Field(default=10000, description="The number of slots in the load balancer algorithm. If the algorithm is set to `round-robin`, this setting determines the maximum number of slots. If the algorithm is set to `consistent-hashing`, this setting determines the actual number of slots in the algorithm. Accepts an integer in the range 10-65536.")
    healthchecks: Optional[CreateUpstreamRequestHealthchecks] = None
    ...
    client_certificate: Optional[CreateUpstreamRequestClientCertificate] = None
    use_srv_name: Optional[StrictBool] = Field(default=None, description="If set, the balancer will use SRV hostname(if DNS Answer has SRV record) as the proxy upstream Host.")
    __properties: ClassVar[List[str]] = ["name", "algorithm", "hash_on", "hash_fallback", "hash_on_header", "hash_fallback_header", "hash_on_cookie", "hash_on_cookie_path", "hash_on_query_arg", "hash_fallback_query_arg", "hash_on_uri_capture", "hash_fallback_uri_capture", "slots", "healthchecks", "tags", "host_header", "client_certificate", "use_srv_name"]
    ...
    def hash_on_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value
    ...
    @field_validator('hash_fallback')
    def hash_fallback_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value
    ...
    def from_dict(cls, obj: Dict) -> Self:
        ...
        _obj = cls.model_validate({
            ...
            "hash_fallback_header": obj.get("hash_fallback_header") if obj.get("hash_fallback_header") is not None else 'none',
